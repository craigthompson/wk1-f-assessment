...
function censorVowels(string) {
  ...
  for (let i = 0; i < string.length; i++) {
    str += vowels.indexOf(string[i]) === -1 ? string[i] : "*";
  }
  return str;
}
...
function stickyCase(string) {
  let str = "";
  for (let i = 0; i < string.length; i++) {
    str += i % 2 === 0 ? string[i].toLowerCase() : string[i].toUpperCase();
  }
  return str;
}
...
function leetspeak(string) {
  const leetspeakLetters = ["a", "e", "i", "o", "s", "t"];
  const leetspeakNums = [4, 3, 1, 0, 5, 7];
  let str = "";
  for (let i = 0; i < string.length; i++) {
