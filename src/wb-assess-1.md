...
The above works because JS will treat parameter c as undefined, so console.log(c) won't execute since the if(c) will evaluate to false, since c is undefined it is falsy.
...
```
Functions can be used to create named blocks of code that can be executed/reused later. They help you keep code DRY and maintainable. DRY meaning don't repeat yourself.
```

**_What is the difference between `console.log` and `return`?_**

```
console.log -- log is a method of console that will output a given message to the console. On the other hand, return will output (return) from a function a given value to the function caller and will end a function's execution.
```

**_What's the difference between defining and calling a function?_**

```
A major difference between definining and calling a function is that defining a function does not execute the function's statements.
Defining a function is where you use the function keyword followed by the name of the function, then a list of parameters (enclosed in parentheses and comma separated), and last the function's statement code to execute (enclosed in {}). It is defining what a function will do, is named, and what values it wants.
Calling a function is different in that you are actually executing a function.
```
...
```
A sequence of characters. String is also a JS object.
```

**_Number_**

```
A value that can be a floating-point number, such as 1.123 or 2.0 or 5.
```
...
```
A JS object that stores a collection of items.
```
